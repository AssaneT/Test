        ...
        <div class="bg-gray-800 shadow-xl h-16 fixed bottom-0 mt-12 md:relative md:h-screen z-10 w-full md:w-48">
            <div class="md:mt-12 md:w-48 md:fixed md:left-0 md:top-0 content-center md:content-start text-left justify-between">
                <ul class="list-reset flex flex-row md:flex-col py-0 md:py-3 px-1 md:px-2 text-center md:text-left">
                    ...
                    <li class="mr-3 flex-1">
                        <a href="{{ route('pageCalendrier') }}" class="block py-1 md:py-3 pl-1 align-middle text-white no-underline hover:text-white border-b-2 border-pink-600">
                            <i class="fas fa-calendar pr-0 md:pr-3 text-pink-600"></i><span class="pb-1 md:pb-0 text-xs md:text-base text-white md:text-white block md:inline-block">Calendrier</span>
                        </a>
                    </li>
                    <li class="mr-3 flex-1">
                    ...
                    <li class="mr-3 flex-1">
                        <a href="{{route('pageSuperAdmin')}}" class="block py-1 md:py-3 pl-0 md:pl-1 align-middle text-white no-underline hover:text-white border-b-2 border-gray-800 hover:border-green-500">
                            <i class="fa fa-user pr-0 md:pr-3"></i><span class="pb-1 md:pb-0 text-xs md:text-base text-gray-600 md:text-gray-400 block md:inline-block">Administrateur</span>
                        </a>
                    </li>
                    <li class="mr-3 flex-1">
        ...
        <div class="main-content flex-1 bg-gray-100 mt-12 md:mt-2 pb-24 md:pb-5">
            ...
            <div class="bg-gray-800 pt-3">
                <div class="rounded-tl-3xl bg-gradient-to-r from-pink-900 to-gray-800 p-4 shadow text-2xl text-white">
                    <h3 class="font-bold pl-2">Calendrier</h3>
                </div>
            </div>
                ...
                @if ($matches->count() > 0)
                    @for ($i = $begin; $i <= $end; $i->modify('+1 day'))
                        @foreach ($matches as $matche)
                            ...
                            @endphp
                            @if ($i == $date)
                                <div class="w-1/3 ml-24 rounded-lg text-2xl bg-pink-900 to-gray-800 text-white py-2 mt-6"><span class="ml-6">{{ $matche->datematch }}</span></div>
                                            ...
                                            <div class="w-2/3 ml-6 border border-gray-300 py-3 flex items-center justify-center hover:bg-blue-100">
                                                ...
                                                @foreach ($equipes as $equipe)
                                                    @if ($equipe->nomequipe == $match->equipedom)
                                                        <img class="object-scale-down w-10 h-8" src="http://localhost/senfoot/storage/app/public/{{ $equipe->logo }}">
                                                    @endif
                                                @endforeach
                                                @if ($date_time_now <= $date_fin)
                                                    <span class="box-border h-15 w-30 p-1 border text-black mr-4 ml-4">{{ $match->heurematch }}</span>
                                                @else
                                                ...
                                                <div class="ml-24 flex justify-end space-x-5">
                                                    <button>
                                                        <a href="{{ route('pageMatch',[$match->championnat_id,$match->id]) }}">
                                                            <span class="underline">Ajouter</span>
                                                        </a>
                                                    </button>
                                                    <button>
                                                        <a href="{{ route('PageModifMatch',[$match->championnat_id,$match->id]) }}">
