    ...
    <nav class="bg-gray-800 pt-2 md:pt-1 pb-1 px-1 mt-0 h-auto w-full z-20 top-0">
        ...
        <div class="box-border h-80 w-full bg-gray-800">
            ...
            <div class="w-full h-52">
                <div class="flex justify-center">
                    <img class="object-left object-contain w-36 h-36 shadow-lg rounded-full max-w-full h-auto align-middle border-none mt-3" src="http://localhost/senfoot/storage/app/public/{{ $equipes->logo }}">
                </div>
                <div class="flex justify-center mt-2 space-x-5">
                    <div class="flex justify-center items-center text-1xl">
                        ...
                        <span class="ml-1 text-white">{{ $equipes->datedecreation }}</span>
                    </div>
                    ...
                    <div class="flex justify-center items-center text-1xl">
                        ...
                        <span class="ml-1 text-white">{{ $equipes->devise }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="w-full container mx-auto flex flex-wrap items-center justify-start py-2">
                    <div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:mt-0 bg-white lg:bg-transparent text-center p-4 lg:p-0">
                        <ul class="list-reset lg:flex justify-start text-white text-2xl flex-1 items-center">
                            <li class="mr-5">
                                <a class="inline-block no-underline hover:text-gray-300 hover:text-underline py-2 px-4" href="{{ route('pageModifEquipe',$equipes->id) }}">General</a>
                            </li>
                            <li class="mr-5">
                                <a class="inline-block no-underline hover:text-gray-300 hover:text-underline py-2 px-4" href="{{route('pageEquipeCalend',$equipes->id)}}">Calendrier</a>
                            </li>
                            <li class="mr-5">
                                <a class="inline-block underline hover:text-gray-300 hover:text-underline py-2 px-4" href="{{ route('pageEquipeEffectif',$equipes->id) }}">Effectif</a>
                            </li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="bg-gray-800">
            <div class="rounded-tl-3xl bg-gradient-to-r from-blue-900 to-gray-800 p-4 shadow text-2xl text-white">
                <h3 class="font-bold pl-2">Liste des joueurs</h3>
            </div>
        </div>

    @if ($message = Session::get('success'))
        <div class="alert alert-success text-center text-2xl text-green-500 mt-5">
            <p>{{ $message }}</p>
        </div>
    @endif

    @if (Session::get('fail'))
        <div class="alert alert-danger text-center text-2xl text-red-500 mt-5">
            {{ Session::get('fail') }}
        </div>
    @endif
    ...
    <center>
        @if ($effectifs->count() > 0)
            <table class="table-auto shadow-lg bg-white mt-10">
                <thead>
                    <tr>
                        <th class="bg-blue-100 border text-center px-6 py-4">PRENOM</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">NOM</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">DATE DE NAISSANCE</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">LIEU DE NAISSANCE</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">NATIONALITE</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">NUMERO</th>
                        <th class="bg-blue-100 border text-center px-6 py-4">POSTE</th>
                        <th class="bg-blue-100 border text-center px-10 py-4">ACTION</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($effectifs as $effectif)
                        @if ($effectif->club == $equipes->nomequipe)
                            <tr>
                                <td class="border px-6 py-5">{{ $effectif->prenom }}</td>
                                <td class="border px-6 py-5">{{ $effectif->nom }}</td>
                                <td class="border px-6 py-5 text-center">{{ $effectif->datenaissance }}</td>
                                <td class="border px-6 py-5 text-center">{{ $effectif->lieunaissance }}</td>
                                <td class="border px-6 py-5 text-center">{{ $effectif->nationalite }}</td>
                                <td class="border px-6 py-5 text-center">{{ $effectif->numero }}</td>
                                <td class="border px-6 py-5 text-center">{{ $effectif->poste }}</td>
                                <td class="border px-10 py-5 text-center">
                                    <a href="{{ route('rompreContrat',$effectif->id) }}" class="border bg-red-500 hover:bg-red-700 p-2 text-white hover:shadow-lg text-xs font-thin">Remove</a>
                                </td>
                            </tr>
                        @endif
                    @endforeach
        ...
        @else
            <span class="mt-8">Pas de joueur enregistrée en base de donnée pour cette équipe</span>
        @endif
    </center>
