...
<body>
            ...
            <div class="flex-1 flex items-center justify-center">
                <div class="hidden sm:block sm:ml-6">
                    <ul class="justify-center items-center flex space-x-14">
                        ...
                        <li>
                            <a href="{{route('clubCalendrier',$equip->id) }}" class="no-underline text-white border-b-2 border-transparent uppercase tracking-wide font-bold text-xs py-3">Calendrier</a>
                        </li>
                        <li>
                            <a href="{{ route('clubStatistique',$equip->id) }}" class="no-underline text-white border-b-2 border-teal-dark uppercase tracking-wide font-bold text-xs py-3">Statistique</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    @foreach ($championnats as $championnat)
        @foreach ($competitions as $competition)
            @if (($championnat->nomchampionnat == $competition->nomcompet) && ($competition->equipe == $equip->nomequipe))
                @php
                    $ok = 1;
                    $id = $championnat->id;
                @endphp
        ...
        @endforeach
        @if ($id != 0)
            <div class="flex justify-center font-semibold text-gray-600 font-serif tracking-widest text-3xl mt-8 mb-3">{{ $championnat->nomchampionnat }}</div>
            <div class="flex justify-center">
                <table class="table-auto shadow-lg bg-white mt-5">
                    <thead>
                        <tr>
                            <!--    <th class="bg-blue-100 border text-left px-10 py-4">LOGO</th>   -->
                            <th class="bg-gray-200 border text-center px-6 py-4">POSITION</th>
                            <th class="bg-gray-200 border text-center px-14 py-4">EQUIPE</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">POINTS</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">JOUÉS</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">GAGNÉS</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">NULS</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">PERDUS</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">BUTS POUR</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">BUTS CONTRE</th>
                            <th class="bg-gray-200 border text-center px-6 py-4">DIFF.</th>
                        </tr>
                    </thead>
                    <tbody>
                        @for ($i = 0; $i <= 20; $i++)
                            @foreach ($classements as $classement)
                                @if (($i == $classement->position) && ($classement->id_championnat == $championnat->id))
                                    <tr>
                                        ...
                                        <td class="inline-flex text-center px-14 py-2">
                                            @foreach ($equipes as $equipe)
                                                @if ($equipe->nomequipe == $classement->equipe)
                                                    <img class="object-scale-down w-8 h-7 mr-2" src="http://localhost/senfoot/storage/app/public/{{ $equipe->logo }}">
                                                @endif
                                            @endforeach
                                            <span class="uppercase font-semibold">{{ $classement->equipe }}</span>
                                        </td>
